...
"""请注意，GCM 模式对加密的明文数据以及仅认证数据输入要求为字节串，不建议使用比特流式输入。"""
...
class GCM:
    ...
    def __init__(self, key : bytes, IV : bytes,  
                 ...
                 len_IV : int = -1):  # 请确保 IV 的 IV_len 个有效位放置在前 IV_len 位（低位的将被截断），并且仅仅只有最后一个字节允许有低位被截断
        '''初始化一个 GCM 对象'''
        len_key = len(key)
        ...
        if len_IV < 0:
            len_IV = len(IV) * 8
        elif len_IV > len(IV) * 8:
            raise ValueError('Invalid IV: len_IV must be less than or equal to the number of bits in IV')
        elif len_IV <= (len(IV) - 1) * 8:
            raise ValueError('Invalid IV: too much bits in IV, please cut IV tail and confirm not uncessary bytes in IV')
            
        

        if len_IV < 64 or len_IV > 1024:
            raise ValueError('Invalid IV: IV length must be 8 to 128 bytes')

        self.__key = key  # 双下划线强私有：密钥
        self.__key_len_bits = len_key * 8  # 双下划线强私有：密钥长度（位）
        self._IV = IV  # 单下划线弱私有：初始向量
        self._IV_len_bits = len_IV  # 单下划线弱私有：初始向量长度（位）
    ...
    def _generate_J0(self) -> bytes:  
        ...
        '''
        if self._IV_len_bits == 96:
            return self._IV + b'\x00\x00\x00\x01'
        else:
            IV_len = self._IV_len_bits.to_bytes(length=8, byteorder='big', signed=False)
            text = self._zero_padding(text = self._IV, block_size = 16, stream_len = self._IV_len_bits)
            data = self._slicing(text = text + b'\x00\x00\x00\x00\x00\x00\x00\x00' + IV_len,
    ...
    # ============================================================ 静态函数 ============================================================ #
    @staticmethod
    def _zero_padding(text : bytes, block_size : int = 16, stream_len : int = -1) -> bytes:
        '''用 0 填充尾部，使其长度为 block_size * 8 的整数倍'''
        if stream_len > 0:
            Len = stream_len
            shift = 4 - (stream_len % 4)
            if shift != 0:
                l = (Len // 8) + 1
            else:
                l = Len // 8
            text_int = (int.from_bytes(text, byteorder='big', signed=False) >> shift) << shift  # 清理比特流式 text 后面的位
            text = text_int.to_bytes(length=l, byteorder='big', signed=False)
        elif stream_len < 0:
            Len = len(text)
        else:
            raise ValueError("stream_len should not be 0")
